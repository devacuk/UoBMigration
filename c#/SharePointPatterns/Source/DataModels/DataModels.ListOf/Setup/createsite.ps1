			...
			$newsite="http://"+$HostName+":" + $WebApp.Port+"/sites/"+ $RISite.Name
					#add Hybrid2 Activities package to farm solutions
							...
							#add solution to solution gallary
								WriteHeading "Step: Add Package($WFSandboxPackage) to solution Gallary."
							...
							#add solution to solution gallary
								WriteHeading "Step4: Add Package($ListOfPkg) to solution Gallary."

								#check of if any existing with that name

								$exitpackage= get-SPUserSolution -Identity $ListOfPkg -site $newsite

								$packagefullpath= $DataModelDir +"\DataModels.ListOf\CreateLink\bin\Debug\"+$ListOfPkg
								
								if ($exitpackage -ne $null)
									{	
										WriteHeading "Removing Existing Package."
										Remove-SPUserSolution -Identity $ListOfPkg -site $newsite -Confirm:$false
									}

								$error.clear()
								WriteHeading "Adding Package from $ListOfPkg to $newsite"
								Add-SPUserSolution -LiteralPath $packagefullpath  -Site $newsite  >>$logfilename
								if ($error) 
									{
										writeError $error[0].Exception 
							...
							$error.clear()
								...
								WriteHeading "Step5: Activate :$ListOfPkg"

							 	Install-SPUserSolution -Identity $ListOfPkg -Site $newsite
								...
								$CrProjSitePkg="CreateProjectSiteListOfWF.wsp"
								...
								$packagefullpath= $DataModelDir +"\DataModels.ListOf\CreateProjectSiteListOfWF\bin\debug\"+$CrProjSitePkg
						...
						Enable-SPFeature -Identity "da0ce02e-b1e9-4378-bbd2-f403054a9cd9" -Url $newsite
						if ($error) 
									{
										writeError $error[0].Exception 
										break;
						...
						WriteHeading "Step Work flow Association with Estimate Content Type."
            						...
            						& "$PublishDataExe" /WFAssociation $newsite "0x010100B022E34803AB43AEA87C937368261B49" "Create Project Site-ListOf" "WFListOfRI" "Tasks" "Workflow History"
							if ($error) 
									{
										writeError $error[0].Exception 
										break;
